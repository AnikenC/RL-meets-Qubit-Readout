...
class EnvParams:
    ...
    max_steps: Optional[int] = 8192  # usually 4096
...
class SinglePhotonLangevinReadoutEnv(SingleStepEnvironment):
    ...
    def __init__(
        ...
        self.grad_dt = self.ts_action[1] - self.ts_action[0]
    ...
    def prepare_action(self, action):
        res_drive = action.astype(jnp.float64)  # Scale up Action
        res_drive = self.normalize_pulse(res_drive)
        res_drive = self.drive_smoother(res_drive)  # Apply Smoother
        res_drive = self.bandwidth_constrainer(res_drive)
        res_drive = self.gradient_clipper(res_drive)  # Apply Gradient Clipper
        res_drive = self.normalize_pulse(res_drive)
        res_drive = self.drive_smoother(res_drive)  # Apply Smoother

        # res_drive = self.bandwidth_constrainer(res_drive)  # Apply Bandwidth Constraints

        return self.a0 * res_drive

    def gradient_clipper(self, res_drive: chex.Array):
        res_drive_diff = jnp.diff(res_drive, n=1) / self.grad_dt
        res_drive_diff_clipped = jnp.clip(
            res_drive_diff, a_min=-self.max_grad, a_max=self.max_grad
        )
        res_drive_processed = jnp.cumsum(
            jnp.concatenate(
                (jnp.array([res_drive[0]]), res_drive_diff_clipped * self.grad_dt)
            )
        )
        return res_drive_processed

    def normalize_pulse(self, res_drive: chex.Array):
        normalizing_factor = jnp.clip(
            self.mu / jnp.absolute(res_drive),
            0.0,
            1.0,
        )
    ...
    def calculate_smoothness(self, action):
        ...
        # New version of smoothness
        # action_acc = jnp.diff(action, n=2)
        # mag_action_acc = jnp.abs(action_acc)
        # mag_action_integrated = trapezoid(mag_action_acc, dx=1.0 / len(mag_action_acc))
    ...
    def extract_values(
        ...
        fidelity = 0.5 * (1.0 + erf(sep * self.scaling_factor))
        fidelity *= 0.5 * (1.0 + decay_g)
